...
import { addCartItem, addMultipleCartItems } from "@/lib/cart.functions";
...
export function ProductDetailsView({ product, onBack }: ProductDetailsViewProps) {
  ...
  const addMultipleCartItemsFn = useServerFn(addMultipleCartItems);
  const downloadPkgFn = useServerFn(downloadProductPackage);

  const [busy, setBusy] = useState(false);
  const [downloadingPkg, setDownloadingPkg] = useState(false);

  // Check if product has Color × Size multidimensional matrix
  const matrixData = useMemo(() => {
    return extractMatrixFromVariants(product.variants, product.slug);
  }, [product.variants, product.slug]);

  const hasColorSizeMatrix = matrixData.colors.length > 0 && matrixData.sizes.length > 0;
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      toast.success(locale === "ar" ? `تمت إضافة ${name} للأوردر` : `Added ${name} to order`);
      void router.navigate({ to: "/checkout" });
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : "Failed to add item to order");
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleDirectDownload = async () => {
    ...
    try {
      ...
      toast.success(
        locale === "ar"
          ? `تم تنزيل ملفات المنتج بنجاح (${res.fileName})`
          : `Product package downloaded (${res.fileName})`,
      );
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : "Failed to download product package");
    } finally {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 items-start">
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          {/* Customer Quantity Matrix (When Color × Size Matrix exists) */}
          {hasColorSizeMatrix ? (
            <div className="space-y-4 pt-1">
              <CustomerQuantityMatrix
                ...
                defaultCashPrice={product.cashPrice}
                defaultPointsPrice={product.defaultPointsPrice}
                busy={busy}
                ctaLabel={locale === "ar" ? "إضافة منتج للأوردر" : "Add Product to Order"}
                onAddSelected={async (selections) => {
                  ...
                  try {
                    ...
                    toast.success(
                      locale === "ar"
                        ? `تمت إضافة ${selections.reduce((acc, s) => acc + s.quantity, 0)} قطعة للأوردر`
                        : `Added ${selections.reduce((acc, s) => acc + s.quantity, 0)} items to order`,
                    );
                    void router.navigate({ to: "/checkout" });
                  } catch (err: unknown) {
                    toast.error(
                      err instanceof Error ? err.message : "Failed to add items to order",
                    );
                  } finally {
                    setBusy(false);
                  }
            ...
            <>
              {/* Dynamic Variant Attributes Selectors for non-matrix products */}
              {dimensions.length > 0 && (
                <div className="space-y-4 pt-1">
                  {dimensions.map((dim) => (
          ...
          {/* Direct Product Package Download Button for Customer / Admin */}
          <Button
            type="button"
            variant="outline"
            size="lg"
            className="w-full gap-2 rounded-xl text-sm font-bold border-primary/30 hover:bg-primary/5 text-primary h-11"
          ...
          >
            ...
            {locale === "ar" ? "تنزيل ملفات المنتج (ZIP)" : "Download Product Package (ZIP)"}
          </Button>
