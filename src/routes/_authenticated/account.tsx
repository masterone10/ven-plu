  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto w-full max-w-4xl space-y-6 px-4 py-10">
        ...
        {/* Product Downloads Section (Rule: Authenticated customer may download any ACTIVE product without purchase) */}
...
function ProductDownloadsCard() {
  const { locale } = useI18n();
  const fetchProducts = useServerFn(listCatalog);
  const downloadPkgFn = useServerFn(downloadProductPackage);
  const [downloadingId, setDownloadingId] = useState<string | null>(null);

  const { data: products, isLoading } = useQuery({
    queryKey: ["account-downloads-catalog"],
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            {locale === "ar" ? "تنزيل ملفات المنتجات" : "Product Downloads"}
          </CardTitle>
          <Badge variant="outline" className="text-xs">
            {locale === "ar" ? "وصول فوري للعملاء" : "Instant Customer Access"}
          </Badge>
        </div>
        <CardDescription>
          {locale === "ar"
            ? "بصفتك عميلاً موثقاً، يمكنك تحميل ملفات وحزم أي منتج نشط في الكتالوج مباشرة."
            : "As an authenticated customer, you can download any active catalog product package directly."}
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <div className="divide-y rounded-lg border">
            {products.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="flex flex-wrap items-center justify-between gap-3 p-3 text-sm hover:bg-muted/40 transition"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {p.variants.length} {locale === "ar" ? "خيارات / متغيّرات" : "variant(s)"}
                      </p>
                    </div>
                  </div>
                  ...
                  <Button
                    ...
                    onClick={() => handleDownload(p.id, name)}
                  >
                    {isDownloading ? (
